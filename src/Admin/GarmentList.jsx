...
const GarmentList = () => {
    ...
    const fetchGarments = async () => {
        try {
            const res = await API.get("/garment/admin/all");
            setGarments(res.data);
        } catch (error) {
            console.error("FETCH ERROR:", error);
            toast.error("Error fetching garments");
        }
    };
    ...
    const handleDelete = async (id) => {
        const result = await Swal.fire({
            title: "Are you sure?",
            text: "This garment will be permanently deleted!",
            icon: "warning",
        ...
        try {
            await API.delete(`/garment/delete/${id}`);

            Swal.fire({
                title: "Deleted!",
                text: "Garment has been deleted successfully.",
                icon: "success",
                confirmButtonColor: "#e2b82e",
                background: "#340B53",
                color: "#e2b82e",
    ...
    const handleUpdate = async () => {
        try {
            ...
            let uploadedImages = editGarment.images || [];

            if (editGarment.newImages) {
                uploadedImages = [];

                for (const img of Array.from(editGarment.newImages)) {
            ...
            let uploadedVideo = editGarment.video || null;

            if (editGarment.newVideo) {
                const res = await uploadToCloudinary(editGarment.newVideo);
                uploadedVideo = {
                    public_id: res.public_id,
                    url: res.secure_url,
                };
            ...
            const payload = {
                ...
                category: editGarment.category,
                description: editGarment.description,
                isActive: editGarment.isActive,
                price: {
                    buy: editGarment.price.buy,
                    rentPerDay: editGarment.price.rentPerDay,
                },
                images: uploadedImages,
                video: uploadedVideo,
            };

            await API.put(`/garment/update/${editGarment._id}`, payload);
    ...
    return (
        <div>
            {/* ================= EDIT MODE ================= */}
            {editGarment ? (
                <div className="max-w-6xl mx-auto backdrop-blur-xl bg-[#340B53] rounded-3xl shadow-2xl border border-white/30 py-5 px-4 text-[#e2b82e]">

                    <h2 className="text-3xl font-bold mb-8">Edit Garment</h2>
                    ...
                    <input
                        ...
                        onChange={(e) =>
                            setEditGarment({ ...editGarment, name: e.target.value })
                        }
                    />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                        ...
                        <input
                            ...
                            className="w-full rounded-xl border border-slate-300 bg-[#340B53] px-4 py-3"
                            value={editGarment.price.rentPerDay}
                            onChange={(e) =>
                                setEditGarment({
                                    ...editGarment,
                                    price: {
                    ...
                    <select
                        className="w-full mb-4 rounded-xl border border-slate-300 bg-[#340B53] px-4 py-3"
                        value={editGarment.category}
                        onChange={(e) =>
                            setEditGarment({ ...editGarment, category: e.target.value })
                        }
                    >
                        {categories.map((cat) => (
                    ...
                    <div className="mb-6">
                        ...
                        {/* Existing Images */}
                        {editGarment.images?.length > 0 && !editGarment.newImages && (
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                                {editGarment.images.map((img, i) => (
                                    <img
                                        key={i}
                        ...
                        {/* New Image Preview */}
                        {editGarment.newImages && (
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
                                {Array.from(editGarment.newImages).map((file, i) => (
                                    <img
                                        key={i}
                    ...
                    <div className="mb-8">
                        ...
                        {/* Existing Video */}
                        {editGarment.video?.url && !editGarment.newVideo && (
                            <video
                                controls
                    ...
                    <div className="flex items-center justify-between mt-8 p-5 rounded-2xl border border-slate-300">
                        <div>
                            ...
                            <p className="text-sm opacity-80">
                                Enable or disable this garment
                            </p>
                        </div>
